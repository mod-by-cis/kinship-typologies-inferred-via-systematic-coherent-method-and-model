...
export function PlotExcel({ data, type, caption, tableClassName, thClassName, tdClassName }: PlotExcelProps): JSX.Element | null {
  if (!data || data.size === 0) {
  ...
  if (type === "col") {
    ...
    return (
      <table className={tableClassName}>
        ...
        <thead>
          <tr>
            {keys.map(key => <th className={thClassName} key={key}>{key}</th>)}
          </tr>
        </thead>
        <tbody>
          {Array.from({ length: maxLength }).map((_, rowIndex) => (
            <tr key={`row-${rowIndex}`}>
              {keys.map(key => {
                ...
                return <td className={tdClassName} key={`${key}-row-${rowIndex}`}>{cellContent}</td>;
              })}
            </tr>
          ))}
        </tbody>
          ...
          {keys.map(key => {
            ...
            return (
              <tr key={`series-row-${key}`}>
                <th scope="row" className={thClassName}>{key}</th> {/* Nagłówek wiersza */}
                {Array.from({ length: maxLength }).map((_, colIndex) => {
                  ...
                  return <td className={tdClassName} key={`${key}-col-${colIndex}`}>{cellContent}</td>;
                })}
              </tr>
            );
          })}
